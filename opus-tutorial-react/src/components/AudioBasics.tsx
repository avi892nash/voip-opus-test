...
const AudioBasics: React.FC = () => {
  ...
  const drawSpectrum = (p: p5) => {
    ...
    const maxHeight = p.height - 60;
    
    for (let i = 0; i < 64; i++) {
      const x = i * barWidth;
      const frequency = i * 0.5;
      const amplitude = Math.sin(p.frameCount * 0.1 + i * 0.2) * 0.5 + 0.5;
      const height = amplitude * maxHeight * (1 - compressionLevel / 100);
      
      // Color based on frequency
      const hue = p.map(i, 0, 64, 0, 255);
      p.fill(hue, 200, 200);
      p.noStroke();
      p.rect(x, p.height - height - 30, barWidth - 1, height);
    }
    ...
    p.textAlign(p.CENTER);
    p.textSize(16);
    p.text('Frequency Spectrum Analysis', p.width/2, 20);
    p.textSize(12);
    p.text('Low Frequency', 50, p.height - 10);
    p.text('High Frequency', p.width - 80, p.height - 10);
  };
