...
// Stable references; defined at module scope so they don't bust useEffect's
// dependency array.
async function loadAudioData(): Promise<AudioData> {
  try {
    const response = await fetch('/learn/sample.wav');
    ...
    // Mono down-mix: take left channel only if stereo.
    const monoSamples = channels === 2 ? samples.filter((_, i) => i % 2 === 0) : samples;

    // Downsample to ~800 visualization points.
    ...
    console.error('Error loading audio, using synthesized fallback:', error);
    return generateFallbackWave();
  }
}

function generateFallbackWave(): AudioData {
  const samples: number[] = [];
  for (let i = 0; i < 800; i++) {
    const t = i / 800;
    const sample =
      Math.sin(2 * Math.PI * 3 * t) * 0.5 +
