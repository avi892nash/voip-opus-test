...
import p5 from 'p5';
import { OpusEncoder, decodeOpusBlob } from '../lib/opusEncoder';
...
const InteractiveDemo: React.FC = () => {
  ...
  // Snapshot of the encoder config at the moment recording started — the .opus
  // file you can download was encoded with these values, even if the user has
  // since moved the sliders.
  const [recordedConfig, setRecordedConfig] = useState<AudioConfig | null>(null);
  ...
  const startRecording = async () => {
    try {
      ...
      // Lock the export snippet + filename to these values; sliders can move
      // freely afterwards without lying about what produced the file.
      setRecordedConfig({ ...config });
  ...
  const stopRecording = async () => {
    try {
      ...
      // Decode the OGG-Opus blob back to PCM via the browser's native decoder
      // so the "compressed" channel of the visualization shows the actual
      // round-tripped audio — not the raw input.
      try {
        const decoded = await decodeOpusBlob(result.compressedBlob);
        setCompressedData(decoded.pcm);
        setDecodeWarning(null);
      } catch (decodeErr) {
        console.warn('Native Opus decode failed, falling back to raw audio for visualization:', decodeErr);
        setCompressedData(result.rawAudio);
        setDecodeWarning(
          'Your browser cannot decode OGG-Opus natively (commonly Safari/iOS). The waveform shown below is the original audio; the size and playback are still accurate.',
        );
      }

      // Store the actual compressed size for the metrics panel.
      const compressedSizeKB = result.compressedAudio.length / 1024;
  ...
  const opusAudioElementRef = useRef<HTMLAudioElement | null>(null);

  const playOpusAudio = async () => {
    if (!compressedBlob || isPlayingOpus) return;

    try {
      setIsPlayingOpus(true);

      // Real round-trip playback: hand the OGG-Opus blob to a plain Audio
      // element. The browser decodes it the same way it would on /call, so
      // the user actually hears what Opus did to their voice.
      const url = URL.createObjectURL(compressedBlob);
      const el = new Audio(url);
      opusAudioElementRef.current = el;

      el.onended = () => {
        setIsPlayingOpus(false);
        URL.revokeObjectURL(url);
        opusAudioElementRef.current = null;
      };
      el.onerror = () => {
        setIsPlayingOpus(false);
        URL.revokeObjectURL(url);
        opusAudioElementRef.current = null;
        alert(
          'Your browser cannot play OGG-Opus directly. Try the WAV button to hear the original.',
        );
      };
      await el.play();
    } catch (error) {
  ...
  /** Standard 16-bit PCM WAV from a Float32 buffer @ 48 kHz mono. */
  function floatToWavBlob(samples: Float32Array, sampleRate = 48000): Blob {
  ...
  function downloadWav() {
    if (!audioData) return;
    triggerDownload(floatToWavBlob(audioData), 'recording.wav');
  }

  function downloadOpus() {
    if (!compressedBlob) return;
    const br = recordedConfig?.bitrate ?? config.bitrate;
    triggerDownload(compressedBlob, `recording-${br}kbps.opus`);
  }

  /** The actual JS that produced the *current* .opus file (locked to the
   *  settings used at record time, not the live sliders). */
  function exportedCodeSnippet(): string {
    const c = recordedConfig ?? config;
    return `import Recorder from 'opus-recorder';

// 1. Build an Opus encoder with the same settings you picked in the demo.
const recorder = new Recorder({
  encoderPath: '/opus-recorder/encoderWorker.min.js',
  encoderApplication: 2049,           // OPUS_APPLICATION_AUDIO
  encoderBitRate:    ${c.bitrate * 1000},${' '.repeat(Math.max(1, 6 - String(c.bitrate * 1000).length))}// ${c.bitrate} kbps
  encoderComplexity: ${c.complexity},${' '.repeat(7)}// 0 (fast) – 10 (high quality)
  encoderFrameSize:  ${c.frameSize},${' '.repeat(7)}// ms
  encoderSampleRate: 48000,
  numberOfChannels:  ${c.channels},
});

// 2. ondataavailable fires once at stop() with the full OGG-Opus stream.
recorder.ondataavailable = (oggOpus) => {
  const blob = new Blob([oggOpus], { type: 'audio/ogg; codecs=opus' });

  // Play it through the browser (real Opus decode):
...
// 3. Run the encoder. start() must be triggered by a user gesture (click/tap).
...
`;
  ...
  async function copyCode() {
    try {
      await navigator.clipboard.writeText(exportedCodeSnippet());
      setCodeCopied(true);
  ...
  const calculateQualityMetrics = () => {
    ...
    // SNR between the raw mic PCM and the Opus-decoded PCM. Compare over the
    // shared length; Opus's frame boundaries may make the decoded buffer a few
    // ms longer or shorter.
    let signalPower = 0;
    let noisePower = 0;
    const n = Math.min(audioData.length, compressedData.length);
    for (let i = 0; i < n; i++) {
    ...
    // Compression ratio in BYTES: 16-bit WAV equivalent vs. the actual
    // OGG-Opus stream size. (The old sample-count ratio compared post-decode
    // PCM to itself — meaningless.)
    const rawBytes = audioData.length * 2; // 16-bit
    const compressedBytes = actualCompressedSize * 1024;
      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
          ...
          >
            ...
            {decodeWarning && recordingComplete && (
              <div className="mb-4 px-4 py-3 rounded-lg bg-yellow-50 border border-yellow-200 text-yellow-800 text-sm">
                {decodeWarning}
              </div>
            )}
            ...
            <div className="flex flex-col items-center space-y-4">
              ...
              {/* Export — download the actual files and show the code that produced them. */}
              {compressedData && audioData && compressedBlob && (
                <div className="w-full max-w-3xl pt-2">
                  <div className="flex flex-wrap items-center gap-3 justify-center">
                    <button
                      onClick={downloadWav}
                      className="px-4 py-2 rounded-lg border border-gray-300 bg-white text-gray-700 text-sm font-medium hover:bg-gray-50 transition-colors inline-flex items-center gap-2"
                    >
                      <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                        <path d="M12 3v10.59l3.3-3.3 1.4 1.42L12 16.41l-4.7-4.7 1.4-1.42 3.3 3.3V3h2zM5 19h14v2H5z" />
                      </svg>
                      Download original .wav
                    </button>
                    <button
                      onClick={downloadOpus}
                      className="px-4 py-2 rounded-lg border border-primary-200 bg-primary-50 text-primary-700 text-sm font-medium hover:bg-primary-100 transition-colors inline-flex items-center gap-2"
                    >
                      <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                        <path d="M12 3v10.59l3.3-3.3 1.4 1.42L12 16.41l-4.7-4.7 1.4-1.42 3.3 3.3V3h2zM5 19h14v2H5z" />
                      </svg>
                      Download .opus ({recordedConfig?.bitrate ?? config.bitrate} kbps)
                    </button>
                    <button
                  ...
                  {showCode && (
                    <div className="mt-4 rounded-xl border border-gray-200 bg-gray-900 text-gray-100 overflow-hidden shadow-medium">
                      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-800">
                        <div className="flex items-center gap-2">
                          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-300 text-[10px] font-semibold uppercase tracking-wide">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                            Live
                          </span>
                          <span className="font-mono text-xs text-gray-400">opus-encode.ts</span>
                        </div>
                        <button
                          onClick={copyCode}
                          className="px-2.5 py-1 rounded text-xs text-gray-200 hover:bg-gray-800 transition-colors"
                        >
                          {codeCopied ? '✓ Copied' : 'Copy'}
                        </button>
                      </div>
                      <pre className="px-4 py-3 text-xs sm:text-[13px] leading-relaxed overflow-x-auto">
                        <code>{exportedCodeSnippet()}</code>
                      </pre>
                      <div className="px-4 py-2 border-t border-gray-800 text-[11px] text-gray-400">
                        This is the exact code that produced the .opus file above. The
                        values are locked to what your sliders were set to when you hit
                        Start Recording — change them and record again to see the snippet
                        update.
                      </div>
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="text-xl font-bold text-primary-500">
                          {Number.isFinite(qualityMetrics.snr)
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="text-xl font-bold text-primary-500">
                          {qualityMetrics.snr >= 30
                            ? 'Excellent'
                            : qualityMetrics.snr >= 20
                            ? 'Good'
                            : qualityMetrics.snr >= 10
                            ? 'Fair'
                            : 'Poor'}
                        </div>
                      </div>
